...
function organise(items: DatasetItem[]): { folders: Folder[]; loose: Leaf[] } {
  ...
  for (const item of items) {
    ...
    leaves.push({ name: item.name, text: display.slice(at + 1), note: item.note })
...
export function FolderDatasetPicker({
  ...
  const countOn = (names: string[]) => names.filter((name) => chosen.has(name)).length
          ...
          return (
            <div className="picker-folder" key={folder.path}>
              <div className="picker-folder-head">
                  ...
                  onClick={() =>
                    setCollapsed((previous) =>
                      previous.includes(folder.path)
                        ? previous.filter((path) => path !== folder.path)
                        : [...previous, folder.path],
                    )
                  }
                >
                  {open ? (
                    <ChevronDown size={14} strokeWidth={2.75} aria-hidden />
                  ) : (
                ...
                >
                  <span className="picker-folder-name">{folder.path}</span>
                </TriCheck>
                <span className="picker-count mono">
                  {on}/{names.length}
                </span>
              </div>
              {open && <div className="picker-leaves">{folder.leaves.map(leafRow)}</div>}
            </div>
          )
        })}
        {loose.map(leafRow)}
...
function TriCheck({
  ...
  return (
    <label className={bare ? 'radio picker-bare' : 'radio check-row'} title={title}>
      <input
        type="checkbox"
        checked={checked}
        disabled={disabled}
