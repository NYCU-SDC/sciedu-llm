...
function withoutSearch(character: PresetCharacter): PresetCharacter {
  return { ...character, tools: character.tools.filter((tool) => tool !== RAG_SEARCH) }
}
...
export function setCourseMaterialMode(
  ...
  if (mode === 'never') {
    return { ...preset, rag_mode: 'off', characters: preset.characters.map(withoutSearch) }
  }
  if (mode === 'always') {
    ...
    return { ...preset, rag_mode: 'forced', characters: preset.characters.map(withoutSearch) }
  }
  const characters = preset.characters.map((character) =>
    character.id === preset.orchestrator && !character.tools.includes(RAG_SEARCH)
...
export function checkPresetShape(value: unknown): ShapeProblem[] {
  ...
  if (!isRecord(value)) return [{ path: '(root)', message: 'must be a JSON object' }]

  if (typeof value.name !== 'string' || !PRESET_ID_PATTERN.test(value.name)) {
    bad('name', 'must be a lowercase slug: letters, digits, "-" and "_", up to 64 characters')
  }
  ...
  if (
    ...
    bad('max_steps', `must be a whole number between 1 and ${MAX_STEPS_CAP}`)
  }
  if (
  ...
  if (value.rag_mode !== undefined && !['off', 'forced'].includes(String(value.rag_mode))) {
    bad('rag_mode', 'must be "off" or "forced"')
  }

  const characters = value.characters
  if (!Array.isArray(characters) || characters.length < 1 || characters.length > 2) {
  ...
  characters.forEach((character, index) => {
    ...
    if (typeof character.id !== 'string' || !PRESET_ID_PATTERN.test(character.id)) {
      bad(`${at}.id`, 'must be a lowercase slug')
    } else {
      ids.push(character.id)
    }
    if (typeof character.display_name !== 'string' || !character.display_name) {
  ...
  if (duplicates.length > 0) {
    bad('characters', `duplicate character ids: ${[...new Set(duplicates)].join(', ')}`)
  }
  if (typeof value.orchestrator !== 'string') {
    bad('orchestrator', 'is required')
  } else if (ids.length > 0 && !ids.includes(value.orchestrator)) {
    bad('orchestrator', `must be one of the character ids: ${ids.join(', ')}`)
  }
...
export function normalisePreset(value: Record<string, unknown>): Preset {
  ...
  return {
    ...
    orchestrator: typeof value.orchestrator === 'string' ? value.orchestrator : 'assistant',
    characters: characters.map((raw): PresetCharacter => {
      ...
      return {
        ...
        role: typeof character.role === 'string' ? character.role : 'assistant',
        prompt_name:
          typeof character.prompt_name === 'string' ? character.prompt_name : null,
        tools: Array.isArray(character.tools)
          ? character.tools.filter((tool): tool is string => typeof tool === 'string')
          : [],
        max_steps: typeof character.max_steps === 'number' ? character.max_steps : 3,
      }
    }),
  }
}
...
export function labelForLoc(loc: (string | number)[], preset: Preset): string {
  ...
  if (head === 'characters' && typeof rest[0] === 'number') {
    ...
    const who = character?.display_name || character?.id || `character ${rest[0] + 1}`
    const field = rest[1]
    const fieldName =
      field === 'prompt_name'
        ? 'prompt'
  ...
  if (typeof head === 'string' && simple[head] && loc.length === 1) return simple[head]
  return path
}
