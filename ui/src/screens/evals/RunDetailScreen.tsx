...
export function RunDetailScreen() {
            ...
            <>
                ...
                <div style={{ marginTop: 20 }}>
                    ...
                    <p className="note" style={{ marginTop: 10 }}>
                        執行清單儲存在服務記憶體中。重新啟動會清空清單，因此重啟前的執行
                        ID 將無法找到；其追蹤紀錄仍在 Langfuse。
                    </p>
                </div>
            </>
        );
    ...
    const sessionUrl = data.session_id
        ? langfuseSessionUrl(data.session_id)
        : null;
                    ...
                    <Panel title="此次執行使用的設定">
                        <table className="table">
                            <tbody>
                                ...
                                <ParamRow
                                    label="受測模型"
                                    value={data.eval_model}
                                    mono
                                />
                                <ParamRow
                                    label="評分模型"
                                    value={data.judge_model}
                                    mono
                                />
                                <ParamRow
                                    label="課程教材"
                                ...
                                <ParamRow
                                    label="題目集"
                                    value={joinNames(data.question_datasets)}
                                    mono
                                />
                                <ParamRow
                                    label="評分提示詞"
                                    value={joinNames(data.judge_prompts)}
                                    mono
                                />
                                <ParamRow
                                    label="每題段落數（k）"
                    ...
                    <Panel title="Langfuse 工作階段">
                        {data.session_id ? (
                            <>
                                ...
                                <p className="note" style={{ marginTop: 10 }}>
                                    每個問題、檢索到的段落、答案與各項分數都會記錄在這個工作階段
                                    ID 下。
                                    {!sessionUrl &&
