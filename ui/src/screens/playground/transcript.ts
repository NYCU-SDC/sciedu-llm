...
import type { AgentEvent, AgentPart, CastCharacter, ChatMessage } from '../../api/agentsStream'
...
export function buildTurn(events: AgentEvent[], end: TurnEnd): AssistantTurn {
  ...
  const open = (agent: string, parent: string | null, summonedBy: string | null) => {
    const nested = (parent !== null || summonedBy !== null) && current !== null
  ...
  const blockFor = (agent: string): SpeakerBlock => {
    if (current && current.agent === agent && !current.closed) return current
    for (let walk: SpeakerBlock | null = current; walk; walk = parentOf.get(walk.key) ?? null) {
  ...
  for (const event of events) {
    switch (event.type) {
      ...
      case 'agent_start': {
        const summoned = event.parent !== undefined || event.summonedBy !== undefined
        ...
        if (!summoned && current && current.agent === event.agent && !current.closed) {
          break
        }
        open(event.agent, event.parent ?? null, event.summonedBy ?? null)
        break
      }
      ...
      case 'part_start': {
        const part: StreamedPart = { ...event.part, index: event.index, streamed: '', done: false }
        parts.push(part)
  ...
  return {
    ...
    blocks,
    parts,
    activeBlockKey: status === 'streaming' && current && !current.closed ? current.key : null,
    status,
    finishReason,
    error,
  }
...
export function hasInternal(block: SpeakerBlock): boolean {
  return block.items.some((item) =>
    item.kind === 'part' ? item.part.internal === true : hasInternal(item.block),
  )
}
 ...
 * spoke. */
export function blockIsVisible(block: SpeakerBlock, showInternal: boolean): boolean {
  if (showInternal) return block.items.length > 0
  return block.items.some((item) =>
    item.kind === 'part' ? item.part.internal !== true : blockIsVisible(item.block, false),
  )
}
...
export function historyMessages(turns: Turn[]): ChatMessage[] {
  ...
  for (const turn of turns) {
    ...
    const content = spokenText(
      buildTurn(turn.events, { aborted: turn.aborted, failed: turn.failure !== null }),
    )
