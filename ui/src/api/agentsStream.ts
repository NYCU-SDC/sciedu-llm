...
export type AgentEvent =
  ...
  | { type: 'agent_start'; agent: string; parent?: string; summonedBy?: string }
...
const PART_TYPES: readonly PartType[] = ['text', 'reasoning', 'tool_call', 'tool_result']
...
function parsePart(value: unknown): AgentPart | null {
  ...
  if (!type || !(PART_TYPES as readonly string[]).includes(type) || !id || !agent) {
    return null
  }
  ...
  if (typeof raw.tool_call_id === 'string') part.tool_call_id = raw.tool_call_id
...
export async function streamAgents(
  run: AgentsRun,
  { signal, onEvent }: { signal: AbortSignal; onEvent: (event: AgentEvent) => void },
): Promise<void> {
  const body = {
